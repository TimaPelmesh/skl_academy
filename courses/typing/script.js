...
// Инициализация состояния сайдбара и иконки
function initSidebarState() {
    if (!sidebar || !content || !menuToggle) return;
    if (window.innerWidth > 991) {
        sidebar.classList.add('active');
        content.classList.add('sidebar-active');
        menuToggle.classList.add('active'); // показываем «крестик», когда меню открыто
    } else {
        sidebar.classList.remove('active');
        content.classList.remove('sidebar-active');
        menuToggle.classList.remove('active'); // на мобильном по умолчанию бургер
    }
}
...
// Тренажер вынесен в отдельную страницу trainer.html
