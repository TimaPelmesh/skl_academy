...
// Кнопка контакта (i) и карточка – авто-добавление для страниц курса Linux
(function contactFabLinux() {
...
(function themeAndColorSwitcher() {
    ...
    const savedTheme = localStorage.getItem('linux-course-theme');
    const savedColorScheme = localStorage.getItem('linux-course-color-scheme');
    ...
    colorSchemeButtons.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            localStorage.setItem('linux-course-color-scheme', colorScheme);
        });
    });
})();
...
(function() {
    ...
    function syncMobileTheme() {
        ...
        const themeLabel = mobileThemeToggle ? mobileThemeToggle.querySelector('.theme-label') : null;
        if (themeLabel) {
    ...
    function syncMobileColorScheme() {
        const activeColor = localStorage.getItem('linux-course-color-scheme');
